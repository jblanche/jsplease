...
featureslug=$(
  echo $feature   |
  # Use sed to replace spaces and dots with hyphens
  sed -E -e "s/\./-/g" -e "s/ +/-/g" -e "s/[^A-Za-z0-9\-]//g" |
...
while [ -z "$tags" ]
do
    tags=$(promptValue "Enter Tags (one or more of: gtie6,gtie7,gtie8,polyfill, fallback, or none)")
    if [ -n "$tags" ]
    then
      set $(echo $tags)
      while [ $# -gt 0 ]
